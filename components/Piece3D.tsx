...
const Piece3D: React.FC<Piece3DProps> = ({ piece, isSelected, onClick, boardRotation }) => {
  ...
  return (
    <div 
      className={`piece-3d relative w-12 h-12 flex items-center justify-center cursor-pointer ${isSelected ? 'z-50' : 'z-10'}`}
      style={{ 
        transform: `rotateZ(${-boardRotation}deg) ${isSelected ? 'translateZ(45px)' : 'translateZ(0px)'}`,
    ...
    >
      ...
      {/* Shadow layer 1: Contact shadow (tight, dark, remains near the base) */}
      {!isSelected && (
        <div 
      ...
      {/* Shadow layer 2: Projected shadow (soft, follows the piece as it lifts) */}
      <div 
      ...
      {/* Top Surface */}
      <div 
        className={`puck-layer absolute inset-0 rounded-full ${topColor} border-2 ${ringColor} flex items-center justify-center shadow-inner ${isKing ? 'animate-king-glow' : ''}`}
        style={{ transform: 'translateZ(12px)' }}
      >
        {isKing && (
          <div className="crown-animation flex items-center justify-center" style={{ transform: 'translateZ(3px)' }}>
             <Crown size={22} className="text-amber-500 fill-amber-500/30" strokeWidth={2.5} />
          </div>
        )}
